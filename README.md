...
func (a *MyApprover) RequestApproval(ctx context.Context, req session.Request) (Decision, error) {
    // req.User, req.Host, req.RemoteIP, req.Command, req.Cwd, req.Digest
    // Return Decision{Allow: true} to allow, Decision{Allow: false} to deny.
    // req.Digest correlates this request with its audit log entries.
    // req.Cwd is the verified working directory queried from the remote session.
